...
namespace ContosoCafeBot
{
    public class CafeBot : IBot
    {
        ...
        public CafeBot(IConfiguration configuration)
        {
            ...
            this.luisModelCafe = new LuisModel(luisModelId, luisSubscriptionKey, luisUri, LuisApiVersion.V2);
        }
            ...
            switch (context.Activity.Type)
            {
                ...
                case ActivityTypes.Message:
                    ...
                    if(!context.Responded)
                    {
                        ...
                        //cafeLUISModel lResult = await lRecognizer.Recognize<ContosoCafeBot.cafeLUISModel>(utterance, ct);//Recognize<cafeLUISModel>(utterance, ct);
                        var untypedResult = await lRecognizer.Recognize(utterance, ct);
                        /* Dictionary<string,object> lD = new Dictionary<string,object>();
                           if(lResult != null) {
                            lD.Add("luisResult", lResult);
                        }*/
                        switch (untypedResult.GetTopScoringIntent().intent)
                        {
                            case "Help":
                                await context.SendActivity("Help intent detected.");
                                break;
                            default:
                                await context.SendActivity(untypedResult.GetTopScoringIntent().intent);
                                break;
                        }
                            // strongly typed
                            /*
        ...
        class LuisTimeOption : ILuisOptions
        {
        ...
        private LuisRecognizer createLUISRecognizer() {
            ILuisOptions luisTimeOption = new LuisTimeOption { TimezoneOffset = 0 };
            
            luisTimeOption.TimezoneOffset = 0;
            return new LuisRecognizer(this.luisModelCafe,
                new LuisRecognizerOptions() { Verbose = true }, luisTimeOption);
        }
